...
csv_config = {'gait': np.array([ 0.        , -1.57079633,  3.14159265,  1.57079633]), 
        'omega_swing': 0.25*7.5398223686155035, 
        'omega_stance': 0.25*21.991148575128552, 
...
class SwimmingNode(Node):
    ...
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(JointCommand, '/hb40/joint_command', qos_pub)
        self.subscriber_ = self.create_subscription(JointState, '/hb40/joint_states', self.subscriber_callback, 10)
        timer_period = CTRL_TIMESTEP  # seconds
        self.kp_ = 2.0
        self.kd_ = .2 
        self.counter_ = 0
    ...
    def get_dummy_trajectory(self):
        xyz = np.zeros((1000,3)) # create a dummy trajectory in cartesian space
        for t in range(1000):
                xyz[t] = np.array([0.1+0.1*np.cos(2*np.pi*t/1000.), HIP_LINK_LENGTH, -0.15+0.1*np.sin(2*np.pi*t/1000.)])
        return xyz
    ...
    def timer_callback(self):
        ...
        """
        hip_angle = 0.0
        thigh_angle = np.pi/2*np.sin(2*np.pi*self.counter_/100.)
        calf_angle = np.pi/2 
